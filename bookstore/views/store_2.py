...
from api.sql import Member, Order_List, Product, Record, Cart, BirdCareGuide
...
@store.route('/birdcareguide/<pid>')
@login_required
def birdcareguide(pid):
    bird_care_data = BirdCareGuide.get_by_pid(pid) 
    
    if not bird_care_data:
        flash("無法找到該商品的護理指南")
        return redirect(url_for('bookstore.bookstore'))
    
    return render_template('birdcareguide.html', data=bird_care_data, user=current_user.name)
